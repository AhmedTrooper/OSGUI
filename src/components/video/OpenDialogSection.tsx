...
export default function OpenDialogSection() {
  ...
  return (
    <div className="w-full grid mt-2  items-center justify-center p-1">
      <Button onPress={()=>setFormatSectionVisible(!formatSectionVisible)} color={formatSectionVisible ? "danger" : "primary"}><span>{formatSectionVisible ? "Close" : "Open"}</span></Button>
    </div>
  );
}
